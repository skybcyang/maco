...
namespace {
    using namespace ro_meta_data;
    __CUB_2_stage_meta_table(rw, ro,
            (a, int),
            (b, double[2]));

    TEST_CASE("test meta") {
        std::cout<<sizeof(ro)<<std::endl;
        rw meta_data;
        REQUIRE(meta_data.a__present() == false);
        REQUIRE(meta_data.b__present() == false);
